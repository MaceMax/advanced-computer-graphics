...
class Geometry : public Node {
   public:
    ...
    Geometry(const std::string& name = "Geometry", std::vector<glm::vec4>& vertices, std::vector<glm::vec3>& normals,
             std::vector<glm::vec2>& texCoords, std::vector<GLuint>& indices, bool useVAO = true) : Node(name),
                                                                                                    m_vertices(vertices),
                                                                                                    m_normals(normals),
